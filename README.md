...
367 vigentes seguem em `Vigentes/ADS/`. A separacao entre ADS e CorsSync ainda
...
`Vigentes/ADS/` ou `Não vigentes/`. Cobre 474 das 484 pastas: as 10 restantes
...
479 projetos na Vercel para 484 pastas, 474 delas vinculadas. As 32 duplicatas
(projetos antigos que serviam o mesmo site de uma pasta ja mapeada) foram
removidas.

Cinco projetos seguem sem pasta no repositorio:

- **Falta o codigo-fonte** — `globotransportes` e `tancervaconveniencia`. Sao
  clientes no ar; o codigo precisa ser trazido para ca.
