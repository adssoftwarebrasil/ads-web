import { Instagram, Phone, MapPin } from 'lucide-react';

const IMG_BASE = 'https://storage.lucasmendes.dev/site-sp/e%20c%20odontologia/img/';

export default function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="bg-black text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          <div className="sm:col-span-2 lg:col-span-1">
            <img
              src={`${IMG_BASE}logo-sem-fundo.webp`}
              alt="E.C. Odontologia Representações"
              className="h-12 w-auto object-contain mb-6"
            />
            <p className="text-gray-400 text-sm leading-relaxed mb-6">
              Soluções completas em equipamentos odontológicos para clínicas e
              consultórios em Belém e região.
            </p>
            <div className="flex items-center gap-3">
              <a
                href="https://www.instagram.com/ec.odontologiarep/"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 border border-white/20 flex items-center justify-center hover:border-white hover:bg-white/10 transition-all duration-200"
                aria-label="Instagram"
              >
                <Instagram size={16} />
              </a>
              <a
                href="http://wa.me/559181685427"
                target="_blank"
                rel="noopener noreferrer"
                className="w-9 h-9 border border-white/20 flex items-center justify-center hover:border-white hover:bg-white/10 transition-all duration-200"
                aria-label="WhatsApp"
              >
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
              </a>
            </div>
          </div>

          <div>
            <h4 className="text-white text-xs font-semibold tracking-[0.2em] uppercase mb-6">
              Produtos
            </h4>
            <ul className="space-y-3 text-gray-400 text-sm">
              {['Cadeiras Odontológicas', 'Autoclaves', 'Compressores', 'Aparelhos de Raio-X', 'Sensores Digitais'].map((item) => (
                <li key={item}>
                  <a
                    href="#produtos"
                    className="hover:text-white transition-colors duration-200"
                  >
                    {item}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h4 className="text-white text-xs font-semibold tracking-[0.2em] uppercase mb-6">
              Navegação
            </h4>
            <ul className="space-y-3 text-gray-400 text-sm">
              {[
                { label: 'Início', href: '#' },
                { label: 'Produtos', href: '#produtos' },
                { label: 'Sobre Nós', href: '#sobre' },
                { label: 'Diferenciais', href: '#diferenciais' },
                { label: 'Contato', href: '#contato' },
              ].map((item) => (
                <li key={item.href}>
                  <a
                    href={item.href}
                    className="hover:text-white transition-colors duration-200"
                  >
                    {item.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          <div>
            <h4 className="text-white text-xs font-semibold tracking-[0.2em] uppercase mb-6">
              Contato
            </h4>
            <ul className="space-y-4">
              <li className="flex items-start gap-3 text-gray-400 text-sm">
                <MapPin size={15} className="text-gray-500 mt-0.5 flex-shrink-0" />
                <span>Av. João Paulo II, 1503<br />Marco, Belém - PA<br />66075-000</span>
              </li>
              <li className="flex items-center gap-3 text-gray-400 text-sm">
                <Phone size={15} className="text-gray-500 flex-shrink-0" />
                <a
                  href="tel:+559181685427"
                  className="hover:text-white transition-colors"
                >
                  (91) 9 8168-5427
                </a>
              </li>
              <li className="flex items-center gap-3 text-gray-400 text-sm">
                <Instagram size={15} className="text-gray-500 flex-shrink-0" />
                <a
                  href="https://www.instagram.com/ec.odontologiarep/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-white transition-colors"
                >
                  @ec.odontologiarep
                </a>
              </li>
            </ul>
          </div>
        </div>
      </div>

      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-gray-600 text-xs text-center sm:text-left">
            © {currentYear} E.C. Odontologia Representações. Todos os direitos reservados.
          </p>
          <p className="text-gray-700 text-xs">
            Belém do Pará, Brasil
          </p>
        </div>
      </div>
    </footer>
  );
}
