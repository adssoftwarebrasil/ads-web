...
export default function Areas() {
  ...
  const heading = site.secoes.areas;
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-5xl mx-auto relative z-10">
        {site.areas.map((area, i) => (
          ...
          >
            ...
            <h4 className="font-display font-bold text-white text-base leading-tight">{area.nome}</h4>
            {area.descricao && (
              <p className="text-white/60 text-xs mt-1.5 leading-relaxed">{area.descricao}</p>
            )}
          </div>
        ))}
      </div>
