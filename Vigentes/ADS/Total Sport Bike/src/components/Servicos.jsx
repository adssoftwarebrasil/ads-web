...
export default function Servicos() {
  if (!siteConfig.servicos.length) return null
  const secao = siteConfig.secoes.servicos
  ...
  return (
    <Section id="servicos" className="bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {siteConfig.servicos.map((servico, i) => {
          const Icone = getIcon(servico.icone)
          return (
            <article
              key={i}
            ...
            >
              ...
              <p className="text-secondary leading-relaxed flex-1 text-[15px]">{servico.descricao}</p>
              <a
