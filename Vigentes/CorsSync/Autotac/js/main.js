const WHATSAPP_PHONE = "5534988551975";
...
function setupServiceTabs() {
  ...
  function setActiveTab(activeTab) {
    ...
    const nextTab = activeTab || fallbackTab;
  ...
  setActiveTab(tabs.find((tab) => tab.classList.contains("is-active")));
...
function setupContactForm() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const lines = [
      "Ola! Vim pelo site da Autotac.",
      "",
      `Nome: ${name || "Nao informado"}`,
      `Telefone: ${phone || "Nao informado"}`,
      `Servico: ${subject || "Nao informado"}`,
