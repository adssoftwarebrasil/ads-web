...
class UltraImageCompressor:
    ...
    def _calculate_adaptive_quality(self, img: Image.Image, analysis: Dict) -> int:
        ...
        if analysis['type'] == 'simple':
            quality -= 8  # Imagens simples comprimem melhor
        elif analysis['type'] == 'gradient':
            quality -= 5  # Gradientes toleram mais compressão
        elif analysis['type'] == 'photo':
            quality += 3  # Fotos precisam mais qualidade
        
        # Ajusta baseado na complexidade
        if analysis['complexity'] > 50:
            quality += 2  # Imagens complexas precisam mais qualidade
        elif analysis['complexity'] < 15:
            quality -= 3  # Imagens simples podem comprimir mais
        
        # Ajusta baseado no contraste
        if analysis['contrast'] < 30:
            quality -= 2  # Baixo contraste comprime melhor
        
        # Mantém dentro dos limites
        return max(70, min(95, quality))
    ...
    def compress(self, input_path: Path, output_path: Path = None) -> Dict:
        ...
        try:
            ...
            # Verifica se pode comprimir mais mantendo qualidade
            compressed_size = output_path.stat().st_size
            if compressed_size > 500 * 1024 and quality > 75:  # > 500KB
                # Tenta qualidade um pouco menor
                test_quality = quality - 3
                test_params = save_params.copy()
                test_params['quality'] = test_quality
                
                test_buffer = io.BytesIO()
                img.save(test_buffer, self.output_format.upper(), **test_params)
                test_size = test_buffer.tell()
                
                # Se redução é significativa (>5%), usa qualidade menor
                if (compressed_size - test_size) / compressed_size > 0.05:
...
class UltraCompressorGUI:
    ...
    def setup_ui(self):
        ...
        # Botões principais (fora do notebook)
        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill=tk.X, pady=(15, 0))
        
        ttk.Button(button_frame, text="📁 Adicionar Imagens", 
        ...
        ttk.Button(button_frame, text="💾 Pasta Destino", 
                  command=self.select_output, style='Big.TButton').pack(side=tk.LEFT, padx=5)
        
        ttk.Button(button_frame, text="🗑️ Limpar Lista", 
        ...
        self.status_label = ttk.Label(status_frame, 
                                     text="✨ Pronto para processar imagens", 
                                     font=('Segoe UI', 9))
    ...
    def _create_config_tab(self, parent):
        ...
        presets = [
            ('Ultra - Compressão máxima inteligente ⭐', 'ultra'),
            ...
            ('Thumbnail - Para miniaturas', 'thumbnail')
        ]
        ...
        quality_frame = ttk.LabelFrame(parent, text="🎚️ Qualidade Manual (Sobrescreve preset)", 
                                      padding="10")
        ...
        quality_scale = ttk.Scale(quality_control, from_=60, to=100, 
                                 variable=self.quality_var, orient=tk.HORIZONTAL, length=250)
        ...
        ttk.Checkbutton(quality_frame, text="Usar qualidade manual", 
                       variable=self.use_custom_quality).pack(anchor=tk.W, pady=(5, 0))
        ...
        ttk.Checkbutton(advanced_frame, text="✨ Qualidade adaptativa (Ajusta automaticamente)", 
                       variable=self.adaptive_quality_var).pack(anchor=tk.W, pady=2)
        
        self.preprocessing_var = tk.StringVar(value='advanced')
        ttk.Checkbutton(advanced_frame, text="🎨 Pré-processamento avançado", 
                       variable=tk.BooleanVar(value=True),
                       command=lambda: self.preprocessing_var.set(
                           'advanced' if self.preprocessing_var.get() == 'none' else 'none')
    ...
    def process_images(self):
        ...
        for i, file in enumerate(self.input_files, 1):
            ...
            if result['success']:
                ...
                self.add_log(f"    📷 Tipo: {result['image_type'].upper()} | "
                           f"Qualidade usada: {result['quality_used']}\n")
                ...
                self.add_log(f"    📁 Salva em: {Path(result['output_path']).name}\n\n")
            else:
                failed_count += 1
                self.add_log(f"❌ [{i}/{total}] {file.name}\n", 'error')
