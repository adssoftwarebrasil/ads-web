        ...
        <div className="flex items-center justify-between h-20">
          <button onClick={() => scrollTo('inicio')} className="flex items-center space-x-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-xs text-gray-600">Sorriso - MT</span>
            </div>
          </button>
          ...
          >
            {menuOpen ? <X className="w-6 h-6" /> : <Menu className="lucide lucide-menu w-6 h-6" />}
          </button>
        </div>
        {menuOpen && (
          <nav className="md:hidden pb-4 flex flex-col space-y-3">
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollTo(item.id)}
                className="text-left text-gray-700 hover:text-[rgb(248,130,31)] transition-colors font-medium py-1"
              >
                {item.label}
              </button>
            ))}
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="bg-gradient-to-r from-[rgb(248,130,31)] to-orange-600 text-white px-6 py-3 rounded-full font-semibold hover:shadow-lg transition-all duration-300 flex items-center justify-center space-x-2"
            >
