...
export default function Contato() {
  ...
  function handleSubmit(e) {
    ...
    const linhas = [
      `Olá, meu nome é ${nome}.`,
      siteConfig.contato.formularioCampoTelefone && telefone ? `Telefone: ${telefone}` : null,
      mensagem,
    ].filter(Boolean)
    window.open(whatsappLink(linhas.join('\n')), '_blank')
  }

  const mostrarFormulario = siteConfig.contato.mostrarFormulario !== false
  const mostrarTelefone = siteConfig.contato.formularioCampoTelefone !== false
    ...
    <Section id="contato" className="bg-gray-50">
      <div className="text-center mb-14">
        ...
        <h2 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-primary mb-4 tracking-tightish text-balance">
          {siteConfig.cta_final.titulo}
        </h2>
        <p className="text-secondary max-w-2xl mx-auto text-base md:text-lg leading-relaxed">
          {siteConfig.cta_final.texto}
        </p>
      </div>

      <div
        className={`grid grid-cols-1 ${
          mostrarFormulario ? 'lg:grid-cols-5' : 'max-w-2xl mx-auto'
        } gap-10 lg:gap-12`}
      >
        <div className={`space-y-6 ${mostrarFormulario ? 'lg:col-span-2 lg:pt-2' : ''}`}>
          <InfoItem icon={<Phone size={22} />} label="Telefone" value={siteConfig.contato.telefone} />
          <InfoItem icon={<Mail size={22} />} label="E-mail" value={siteConfig.contato.email} />
          {siteConfig.contato.endereco && (
            <InfoItem icon={<MapPin size={22} />} label="Endereço" value={siteConfig.contato.endereco} />
          )}
          {siteConfig.contato.horario && (
            <InfoItem icon={<Clock size={22} />} label="Horário" value={siteConfig.contato.horario} />
          )}
          <a
            href={whatsappLink()}
            target="_blank"
        ...
        {mostrarFormulario && (
          ...
          >
            ...
            >
              <Send size={18} /> {siteConfig.cta_final.botao}
            </button>
      ...
      {siteConfig.contato.mapaEmbed && (
        <div className="mt-14 md:mt-20">
          ...
          <div className="rounded-3xl overflow-hidden shadow-ring h-80 md:h-96 border border-gray-100">
            <iframe
              src={siteConfig.contato.mapaEmbed}
              width="100%"
              height="100%"
              style={{ border: 0 }}
              allowFullScreen
